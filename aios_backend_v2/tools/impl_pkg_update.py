...
from ..util.pkg_commands import build_update_command, run_command


class PkgUpdate(Tool):
    name = "pkg_update"
    description = "Update/refresh an app/package from the allowlist."
    permissions = ["pkg:update"]
    params_schema = {
        "type": "object",
        "properties": {
            "name": {"type": "string"},
    ...
    async def run(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not canon:
            return {"ok": False, "error": f"'{raw}' not in update allowlist"}
        channel = choose_channel(canon, channel_hint)
        if not channel:
            return {"ok": False, "error": "No supported channel"}
        commands = build_update_command(canon, channel)
        apply_cmd = commands.get("apply")
        if not apply_cmd:
            return {"ok": False, "error": "Unable to build update command"}
        ...
        if dry_cmd:
            ...
            if rc != 0:
                return {
                    ...
                    "dry_run": dry_result,
                    "error": "Dry-run failed; aborting update",
                }
        rc, out, err = run_command(apply_cmd)
        note = f"Updated {get_display_name(canon)} via {channel}"
        return {
            "ok": rc == 0,
            "channel": channel,
            "name": canon,
